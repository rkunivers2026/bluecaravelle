...
Placez le `index.html` autonome et le dossier `assets/` dans `server/public/` :

```
server/public/index.html      ← front (build autonome)
server/public/assets/…        ← images des plats (mode API)
```

Le serveur les sert alors sur `/`, et le front parle à l'API en même origine
(aucun CORS à configurer). C'est déjà pré-rempli dans ce livrable.
